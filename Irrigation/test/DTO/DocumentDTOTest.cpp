...
TEST(DocumentDTOTest, parametrizedConstructor) {
	const list<ProgramDTO> expectedPrograms({
		ProgramDTO(true, "abc", 83, "sch1", EveryDaySchedulerDTO(), PeriodicSchedulerDTO(), WeeklySchedulerDTO(), list<RunTimeDTO>({ RunTimeDTO() }), list<StartTimeDTO>()),
		ProgramDTO(false, "123", 125, "sch2", EveryDaySchedulerDTO(), PeriodicSchedulerDTO(), WeeklySchedulerDTO(), list<RunTimeDTO>(), list<StartTimeDTO>({ StartTimeDTO()}))
	});
