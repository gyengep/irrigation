...
void XmlWriter::saveTemperatureCurrent(pugi::xml_node* parent, const TemperatureCurrentDTO& temperatureCurrent) {
	xml_node node = parent->append_child("temperature");

	node.append_child("value").text().set(static_cast<int>(temperatureCurrent.getValue()));
	node.append_child("datetime").text().set(temperatureCurrent.getDateTime().c_str());
	node.append_child("unit").text().set(temperatureCurrent.getUnit().c_str());
}
...
std::string XmlWriter::save(const TemperatureCurrentDTO& temperatureCurrent, const std::string& styleSheet) {
	unique_ptr<xml_document> doc(new xml_document());
	prependStyleSheet(doc.get(), styleSheet);

	saveTemperatureCurrent(doc.get(), temperatureCurrent);
	return toString(doc.get(), humanReadable);
}
